...
using UnityEngine;
using ZM.UI;
using ZM.AssetFrameWork;  // Added missing semicolon
...
public class UIModule
{
    ...
    #region 智能显隐
    private bool mSmartShowHide = false;//智能显隐开关（可根据情况选择开启或关闭）
    ...
    public void HideAllWindow()
    {
        ...
        {
            HideWindow(mVisibleWindowList[i].Name);
        }
    ...
    {
        //从配置文件获取窗口的路径
        string path = mWindowConfig.GetWindowData(wndName).path;
        // Remove "Assets/Resources/" from the path since Resources.Load needs relative path
        //path = path.Replace("Assets/Resources/", "");
        GameObject window = ResourcesLoadObj(path, mUIRoot);
    ...
    public GameObject ResourcesLoadObj(string wndNamepath, Transform parent)
    {
        //return null;
        //Debug.Log("ResourcesLoadObj:" + wndName);
        //Debug.Log("ResourcesLoadObj:" + parent.gameObject.name);
        if (wndNamepath== "Assets/Resources/prefabs/Window/FailureWindow")
        {
            wndNamepath = wndNamepath.Replace("Assets/Resources/", "");
            return GameObject.Instantiate<GameObject>(Resources.Load<GameObject>(wndNamepath), parent);
        }
        //return GameObject.Instantiate<GameObject>(Resources.Load<GameObject>(wndNamepath), parent);
        return ZMAssetsFrame.Instantiate(wndNamepath, parent, Vector3.zero, Vector3.one, Quaternion.identity);
        //在这里替换成自己的资源加载框架 例:
