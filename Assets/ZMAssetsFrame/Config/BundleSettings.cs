...
public class BundleSettings : ScriptableObject
{
    ...
    {
        //返回一个字符串，字符串内容为：资源内嵌的路径 unity的streamingAssetsPath + "/AssetBundle/"+ 资源模块类型 + 斜杠
    ...
    {
        //返回一个字符串，字符串内容为：解压文件的路径 资源解压路径 unity的持久化目录+ "/DecompressAssets/" + 资源模块类型 + 斜杠
    ...
    {
        //返回一个字符串，字符串内容为：热更文件储存路径  unity的持久化目录+ "/HotAssets/" + 资源模块类型 + 斜杠
