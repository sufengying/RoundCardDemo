...
namespace ZM.AssetFrameWork
{
    ...
    public class ResourceManager : IResourceInterface
    {
            ...
            if (mLoadObjectCallBackDic.Count > 0)
            {
                ...
                foreach (var item in mLoadObjectCallBackDic)
                {
                    if (ListContainsAsset(assetsItemList, item.Value.crc))
                    {
                        ...
                        item.Value.loadResult?.Invoke(
                                            // 实例化对象   
                                            Instantiate(item.Value.path, 
                                                        null, 
                                                        Vector3.zero, 
                                                        Vector3.one, 
                                                        Quaternion.identity),
        ...
        /// <summary>
        /// 判断列表中是否包含该资源，通过crc路径
        /// </summary>
        /// <param name="assetsItemList"></param>
        ...
        {
            //如果path不以.prefab结尾，则添加.prefab结尾
        ...
        public T LoadResource<T>(string path) where T : UnityEngine.Object
        {
            ...
            {
                //加载该路径对应的AssetBundle
                //通过crc值，获取到对应的AssetBundle，并创建一个BundleItem对象返回
                item = AssetBundleManager.Instance.LoadAssetBundle(crc);
                if (item != null)
                {
                    //如果item的assetBundle不为空
                    if (item.assetBundle != null)
                    {
                        //如果item的obj不为空，则直接返回item的obj
                        //LoadFromFile：加载AssetBundle文件本身
                        //LoadAsset<T>：从已加载的AssetBundle中加载具体资源
        ...
        public BundleItem GetCacheItemFormAssetDic(uint crc)
        {
            ...
            //计算资源路径的CRC32值作为唯一标识
            //通过Crc32，在已经加载过的资源字典 key为资源路径Crc vluae 为资源对象获取对象
            mAlreayLoadAssetsDic.TryGetValue(crc, out item);
            //如果item为空，则创建一个BundleItem对象
